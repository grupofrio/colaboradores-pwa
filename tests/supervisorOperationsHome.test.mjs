...
// El check anti-fuga es ÚNICO y canónico (`check_supervisor_day_control_leak.mjs`,
// ya cableado al build); cubre también los marcadores de la home "Hoy".
import {
  findSupervisorDayControlLeaks as findDayControlFixtureLeaks,
  SUPERVISOR_DAY_CONTROL_BANNED as DAYCONTROL_FIXTURE_MARKERS,
...
// Contrato canónico del harness en main: devuelve { Component, mod, cleanup } y
// espera una ruta ABSOLUTA (mismo idiom que m4UnavailableState.test.mjs).
const { Component: View } = await loadJsxDefault(fileURLToPath(
  new URL('../src/modules/supervisor-ventas/dayControl/OperationsHomeView.jsx', import.meta.url),
))
...
test('leak-check: detecta un marcador del fixture en un asset simulado', () => {
  ...
  assert.equal(leaks[0].sentinel, marker)
})

test('leak-check: cubre los marcadores propios de la home "Hoy"', () => {
  // El check canónico DEBE morder también con los rótulos del demoLoader de "Hoy".
  for (const marker of ['Chofer Demo Uno', 'Ruta Demo Dos', 'Unidad Demo A']) {
    assert.ok(DAYCONTROL_FIXTURE_MARKERS.includes(marker), `falta centinela ${marker}`)
    const leaks = findDayControlFixtureLeaks([{ name: 'chunk.js', content: `x="${marker}"` }])
    assert.equal(leaks.length, 1)
  }
})

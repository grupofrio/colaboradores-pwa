...
// ── MGR-GAP-006 (no-regresión mínima) ────────────────────────────────────────
// El stack draft del shell "Mi Sucursal" se basó en historia pre-Copiloto
// Gerencial y NO declara `/gerente/copiloto`. Al portar el shell a `main`, esa
// ruta (y la de Copiloto) deben seguir intactas. Cobertura completa en
// tests/gerenteCopilotoNoRegression.test.mjs cuando exista ese archivo.
test('MGR-GAP-006: portar el shell V2 NO retira /gerente/copiloto de App.jsx', () => {
  const app = src('../src/App.jsx')
  assert.match(app, /ScreenCopilotoGerencial/)
  assert.match(app, /path="\/gerente\/copiloto"/)
  assert.match(app, /moduleId="copiloto_gerencial"/)
})

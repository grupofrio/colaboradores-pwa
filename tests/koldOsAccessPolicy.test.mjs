// Codex ronda 2 §6 — POLÍTICA CANÓNICA DE ACCESO (M1 Tower + M2 + M3 juntos).
//
// Estos tests existen porque los tres módulos llegaron por PRs distintos y cada
...
// Los 3 módulos KOLD OS y sus superficies, en una sola tabla.
...
test('ACCESS_POLICY_RESOLVERS registra m2 y m3; Tower NO entra al registro', () => {
  assert.deepEqual(Object.keys(ACCESS_POLICY_RESOLVERS).sort(), ['m2', 'm3'])
...
// ── Matriz §6 A–F ────────────────────────────────────────────────────────────
test('A. Aida (tower_status=supervisor_ventas): ve Tower, NO ve M2/M3', () => {
  ...
  for (const id of ['planeacion', 'ejecucion']) {
...
test('B. admin_plataforma: ve Tower + M2 + M3 y ENTRA a los tres', () => {
  const sess = tower('admin_plataforma')
  const v = surfaces(sess)
  for (const id of ['torre_operativa', 'planeacion', 'ejecucion']) {
  ...
  for (const mod of [TOWER, M2, M3]) {
    assert.ok(entersInto(mod, sess), `entra a ${mod.id}`)
  }
})

test('C. direccion_general SIN tower_status: NO ve Tower, SÍ ve M2/M3', () => {
  const sess = s('direccion_general')
  const v = surfaces(sess)
  assert.ok(!v.home.includes('torre_operativa'), 'Tower exige tower_status, no x_job_key')
  assert.equal(entersInto(TOWER, sess), false)
  assert.ok(v.home.includes('planeacion') && v.home.includes('ejecucion'))
  assert.ok(entersInto(M2, sess) && entersInto(M3, sess))
})
...
test('D. gerente_sucursal: no ve ni entra a ninguno de los tres (v1)', () => {
  ...
  for (const mod of [TOWER, M2, M3]) assert.equal(entersInto(mod, sess), false)
})
...
test('las 3 rutas KOLD OS existen una sola vez y no colisionan', () => {
  for (const route of ['/torre/backlog', '/planeacion', '/ejecucion']) {

    ...
    <div style={{
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 20, paddingBottom: 16 }}>
          ...
          <span style={{ ...typo.title, color: UI.colors.textSoft }}>Ciclo de Congelacion</span>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: 24, textAlign: 'center', ...typo.body, color: UI.colors.textMuted }}>
            Sin turno activo
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                return (
                  <div key={s} style={{ flex: 1 }}>
                    <div style={{
                      ...
                      background: done ? UI.colors.success : active ? UI.colors.blue2 : UI.colors.border,
                    }} />
                    <p style={{ ...typo.caption, margin: 0, textAlign: 'center', color: active ? UI.colors.blue2 : done ? UI.colors.success : UI.colors.textLow, fontWeight: active ? 700 : 500 }}>
                      {s}
                    </p>
                  </div>
                )
            ...
            {step === 'start' && (
              <div style={{
                ...
                <p style={{ ...typo.overline, color: isLightSurface ? 'rgba(255,255,255,0.82)' : UI.colors.textLow, marginBottom: 8 }}>
                  CICLO #{(cycles.filter(c => c.state === 'dumped').length) + 1}
                </p>
                <p style={{ ...typo.h1, color: isLightSurface ? UI.colors.onPrimary : UI.colors.text, margin: 0, marginBottom: 8 }}>
                  ¿Empezo a congelar?
                </p>
                <p style={{ ...typo.caption, color: isLightSurface ? UI.colors.onPrimary : UI.colors.textMuted, opacity: isLightSurface ? 0.86 : 1, marginBottom: 20 }}>
                ...
                <div style={{
                  ...
                  <p style={{ ...typo.caption, margin: 0, fontWeight: 700, color: totalBagsAvailable > 0 ? UI.colors.success : UI.colors.warning }}>
                    {totalBagsAvailable > 0 ? `${totalBagsAvailable} bolsas disponibles para producir` : 'No hay bolsas disponibles en el turno'}
                  </p>
                  <p style={{ ...typo.caption, color: UI.colors.textMuted, margin: '4px 0 0' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: totalBagsAvailable > 0 ? 'linear-gradient(90deg, #15499B, #2B8FE0)' : UI.colors.surface,
                    color: totalBagsAvailable > 0 ? 'white' : UI.colors.textLow,
            ...
            {step === 'freezing' && progress && (
              <div style={{
                ...
                background: `linear-gradient(180deg, ${UI.colors.blue2}18, ${UI.colors.surfaceSoft})`,
                border: `1px solid ${UI.colors.blue2}30`,
              }}>
                <p style={{ ...typo.overline, color: UI.colors.textLow, marginBottom: 4 }}>
                  CICLO #{activeCycle.cycle_number || '?'} · CONGELANDO
                </p>
                <div style={{ textAlign: 'center', margin: '16px 0' }}>
                  <p style={{
                    fontSize: 44, fontWeight: 700, margin: 0, letterSpacing: '-0.04em',
                    color: progress.isOverdue ? UI.colors.error : UI.colors.text,
                  ...
                  <p style={{ ...typo.caption, color: UI.colors.textMuted, margin: 0, marginTop: 4 }}>
                    {progress.isOverdue ? 'Tiempo cumplido' : `Faltan para terminar (de ${progress.expectedMin} min)`}
                  </p>
                </div>
                <div style={{ height: 10, borderRadius: 5, background: UI.colors.border, overflow: 'hidden', marginBottom: 16 }}>
                  <div style={{
                    height: '100%', borderRadius: 5, transition: 'width 1s linear',
                    width: `${progress.progressPct}%`,
                    background: progress.isOverdue ? UI.colors.error : progress.progressPct >= 80 ? UI.colors.warning : UI.colors.blue2,
                  }} />
                </div>
                <p style={{ ...typo.caption, color: UI.colors.textMuted, textAlign: 'center', marginBottom: 16 }}>
                ...
                {freezeWarning && (
                  <div style={{
                    ...
                    background: UI.colors.warningSoft, border: '1px solid rgba(245,158,11,0.3)',
                  }}>
                    <p style={{ ...typo.caption, color: UI.colors.warning, margin: 0, fontWeight: 600 }}>
                      {freezeWarning}
                    </p>
                  </div>
                )}
            ...
            {step === 'defrosting' && progress && (
              <div style={{
                padding: 20, borderRadius: TOKENS.radius.xl,
                background: `linear-gradient(180deg, ${UI.colors.warning}18, ${UI.colors.surfaceSoft})`,
                ...
                <div style={{ textAlign: 'center', margin: '16px 0' }}>
                  <p style={{
                    fontSize: 44, fontWeight: 700, margin: 0, letterSpacing: '-0.04em',
                    color: progress.isOverdue ? UI.colors.success : UI.colors.warning,
                  }}>
                ...
                <div style={{ height: 10, borderRadius: 5, background: UI.colors.border, overflow: 'hidden', marginBottom: 16 }}>
                  <div style={{
                    height: '100%', borderRadius: 5, transition: 'width 1s linear',
                    width: `${progress.progressPct}%`,
                    background: progress.isOverdue ? UI.colors.success : UI.colors.warning,
                  }} />
                </div>
                ...
                <div style={{
                  ...
                  <p style={{ ...typo.overline, color: UI.colors.textLow, marginBottom: 6 }}>DESCARGA BASE DEL CICLO</p>
                  <p style={{ fontSize: 28, fontWeight: 700, color: UI.colors.text, margin: 0 }}>

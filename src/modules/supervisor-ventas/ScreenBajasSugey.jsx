...
import { getSugeyDeactivationQueue } from './customerDeactivationService'
...
export default function ScreenBajasSugey() {
  ...
  const load = useCallback(async (filters = {}) => {
    ...
    setError('')
    try {
      const payload = await getSugeyDeactivationQueue({ limit: 50, offset: 0, ...filters })
      ...
      setError(e?.message || 'No se pudo cargar la cola de Sugey')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <ScreenShell title="Verificacion Sugey" backTo="/equipo/bajas">
      ...
      {loading ? (
        <Loader label="Cargando revisitas" />
      ) : error ? (
        <ErrorState message={error} onRetry={() => load({ route, reason })} />
      ) : requests.length === 0 ? (
        <EmptyState icon="📋" title="Sin revisitas" subtitle="No hay bajas pendientes para Sugey" typo={typo} />
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 14 }}>
          {requests.map((request) => (
            <button
              key={request.id}
              onClick={() => navigate(`/equipo/bajas/sugey/${request.id}`)}
              style={{
                width: '100%',
                padding: 14,
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start' }}>
                ...
                <span style={{ ...typo.caption, color: '#14b8a6', fontWeight: 700 }}>
                  Verificar
                </span>
              </div>
              <p style={{ ...typo.caption, margin: '8px 0 0', color: TOKENS.colors.textSoft }}>
                ...
                {request.request_photo_url ? ' · Con foto inicial' : ''}

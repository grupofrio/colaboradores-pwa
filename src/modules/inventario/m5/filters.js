// ─── KOLD OS · M5 — Filtros locales del drill-down (puros) ───────────────────
// MISMA semántica de parámetros que GET /pwa-kold-os/m5/findings (backend
// gf_kold_os_m5). En producción el filtrado/paginado es SERVER-SIDE; este
// módulo se usa para el modo demo (fixture) y como referencia del contrato.

export const M5_DEFAULT_FILTERS = Object.freeze({
  category: '',
  severity: '',
  verdict: '',
  classification: '',
...
export function applyFindingFilters(findings = [], filters = M5_DEFAULT_FILTERS) {
  const f = { ...M5_DEFAULT_FILTERS, ...(filters || {}) }
...
export function paginate(items = [], page = 1, pageSize = M5_PAGE_SIZE) {
  const list = Array.isArray(items) ? items : []
  const size = Number.isInteger(pageSize) && pageSize > 0 ? pageSize : M5_PAGE_SIZE

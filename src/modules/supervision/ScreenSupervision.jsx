...
const STATUS_COLORS = {
  ok: TOKENS_LIGHT.colors.success,
  warning: TOKENS_LIGHT.colors.warning,
  alerta: TOKENS_LIGHT.colors.error,
  info: TOKENS_LIGHT.colors.textMuted,
}
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 20, paddingBottom: 12 }}>
          <button onClick={() => navigate('/')} aria-label="Volver" style={{
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={TOKENS_LIGHT.colors.textSoft} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5"/><path d="M12 19l-7-7 7-7"/></svg>
          </button>
          <span style={{ ...typo.title, color: TOKENS_LIGHT.colors.textSoft }}>Supervisión de Producción</span>
          <button onClick={loadData} aria-label="Refrescar" style={{
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={TOKENS_LIGHT.colors.textSoft} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10"/><path d="M20.49 15a9 9 0 0 1-14.85 3.36L1 14"/></svg>
          </button>
        </div>
        ...
        {flashMessage && (
          <div style={{
            ...
            <span style={{ ...typo.caption, color: flashMessage.type === 'success' ? TOKENS_LIGHT.colors.success : TOKENS_LIGHT.colors.error }}>
              {flashMessage.text}
            </span>
          </div>
        )}
          ...
          <>
            ...
            {tanks.length > 0 && (
              <div style={{ marginTop: 18 }}>
                <p style={{ ...typo.overline, color: TOKENS_LIGHT.colors.textLow, marginBottom: 10 }}>ESTADO DE BARRAS</p>
                {tanks.map(t => (
                  <TankRow
                    key={t.id}
                    t={t}
            ...
            <p style={{ ...typo.overline, color: TOKENS_LIGHT.colors.textLow, marginTop: 24, marginBottom: 12 }}>GESTIÓN</p>
            <NavGrid
              items={[
                { id: 'turno', label: 'Control de Turno', desc: stateLabel(shift.state), route: '/supervision/turno', color: TOKENS_LIGHT.colors.blue3 },
                { id: 'paros', label: 'Paros de Línea', desc: countBadge('paro', summary.open_downtimes), route: '/supervision/paros', color: TOKENS_LIGHT.colors.error },
                { id: 'energia', label: 'Energía', desc: energyLabel(summary.energy_kwh, shift.energy_kwh), route: '/supervision/energia', color: TOKENS_LIGHT.colors.blue2 },
                { id: 'merma', label: 'Merma', desc: mermaLabel(summary.total_scrap_kg, shift.total_scrap_kg), route: '/supervision/merma', color: TOKENS_LIGHT.colors.warning },
...
function EmptyShift({ typo, onOpen }) {
  return (
    <div style={{ marginTop: 40, padding: 24, borderRadius: TOKENS.radius.xl, background: TOKENS_LIGHT.colors.surface, border: `1px solid ${TOKENS_LIGHT.colors.border}`, textAlign: 'center' }}>
      <div style={{ fontSize: 40, marginBottom: 12 }}>&#x1F3ED;</div>
      <p style={{ ...typo.title, color: TOKENS_LIGHT.colors.text }}>Sin turno activo</p>
      <p style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, marginTop: 6 }}>Abre un turno para comenzar a registrar.</p>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ ...typo.overline, color: TOKENS_LIGHT.colors.textLow, margin: 0 }}>{shift.name || `Turno ${shift.shift_code}`}</p>
          <p style={{ ...typo.title, color: TOKENS_LIGHT.colors.text, margin: '2px 0 0' }}>{title}</p>
          <p style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, margin: '2px 0 0' }}>{subtitle}</p>
        </div>
        {cta && (
          <button onClick={cta.onClick} style={{
            padding: '8px 14px', borderRadius: TOKENS.radius.pill,
            background: color, color: TOKENS_LIGHT.colors.onPrimary, fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap',
          }}>{cta.label}</button>
        )}
      </div>
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <p style={{ ...typo.overline, color: TOKENS_LIGHT.colors.textLow, margin: 0 }}>MÉTRICAS DEL TURNO</p>
        {score !== null && (
          <span style={{ padding: '2px 8px', borderRadius: TOKENS.radius.pill, background: 'rgba(34,197,94,0.12)', border: '1px solid rgba(34,197,94,0.25)', fontSize: 11, fontWeight: 700, color: TOKENS_LIGHT.colors.success }}>Score {score.toFixed(0)}</span>
        )}
      </div>
      ...
      {balance !== null && (
        <div style={{ marginTop: 10, padding: '8px 10px', borderRadius: TOKENS.radius.sm, background: TOKENS_LIGHT.glass.panelSoft, border: `1px solid ${TOKENS_LIGHT.colors.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted }}>Desbalance</span>
          <span style={{ ...typo.caption, color: balance > (summary.balance_blocker_threshold_pct || 5) ? TOKENS_LIGHT.colors.error : balance > (summary.balance_warning_threshold_pct || 1) ? TOKENS_LIGHT.colors.warning : TOKENS_LIGHT.colors.success, fontWeight: 700 }}>
            {balance.toFixed(1)}%
          </span>
        </div>
      )}
...
function Kpi({ label, value, warn, typo }) {
  return (
    <div style={{ padding: '8px 6px', borderRadius: TOKENS.radius.sm, background: TOKENS_LIGHT.glass.panelSoft, border: `1px solid ${TOKENS_LIGHT.colors.border}`, textAlign: 'center' }}>
      <p style={{ fontSize: 9, fontWeight: 600, color: TOKENS_LIGHT.colors.textMuted, margin: 0, letterSpacing: '0.1em' }}>{label.toUpperCase()}</p>
      <p style={{ fontSize: 16, fontWeight: 700, color: warn ? TOKENS_LIGHT.colors.warning : TOKENS_LIGHT.colors.text, margin: '2px 0 0' }}>{value}</p>
    </div>
  )
}
...
function Section({ title, children, typo }) {
  return (
    <div style={{ marginTop: 16 }}>
      <p style={{ ...typo.overline, color: TOKENS_LIGHT.colors.textLow, marginBottom: 8 }}>{title.toUpperCase()}</p>
...
function IssueRow({ level, code, message, onGo, goLabel, typo }) {
  const color = STATUS_COLORS[level] || TOKENS_LIGHT.colors.textMuted
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textSoft, margin: 0, fontWeight: 600 }}>{message}</p>
        {code && <p style={{ fontSize: 9, color: TOKENS_LIGHT.colors.textMuted, margin: '2px 0 0', letterSpacing: '0.05em' }}>{code}</p>}
      </div>
      {onGo ? (
        <button onClick={onGo} style={{
          padding: '6px 12px', borderRadius: TOKENS.radius.pill,
          background: color, color: TOKENS_LIGHT.colors.onPrimary, fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap',
        }}>{goLabel || 'Ir a corregir'}</button>
      ) : (
        <span style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, fontStyle: 'italic' }}>—</span>
      )}
    </div>
  )
}
...
function TankRow({ t, typo, onClick, onRegisterSalt }) {
  ...
  return (
    <div style={{
      ...
      background: hasAlerts ? 'rgba(239,68,68,0.06)' : TOKENS_LIGHT.glass.panel,
      border: `1px solid ${hasAlerts ? 'rgba(239,68,68,0.25)' : TOKENS_LIGHT.colors.border}`,
      ...
      <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%', textAlign: 'left' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
          <p style={{ ...typo.body, color: TOKENS_LIGHT.colors.text, margin: 0, fontWeight: 700 }}>{t.display_name || t.name}</p>
          <span style={{
            padding: '2px 8px', borderRadius: TOKENS.radius.pill, fontSize: 11, fontWeight: 700,
            background: (t.ready_slots_count || 0) > 0 ? TOKENS_LIGHT.colors.successSoft : TOKENS_LIGHT.colors.surfaceSoft,
            color: (t.ready_slots_count || 0) > 0 ? TOKENS_LIGHT.colors.success : TOKENS_LIGHT.colors.textMuted,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
          ...
          <TankMini label="TEMP" value={t.brine_temp ? `${t.brine_temp}°C` : '—'} color={tempBad ? TOKENS_LIGHT.colors.error : tempOk ? TOKENS_LIGHT.colors.success : null} />
          <TankMini label="SAL" value={t.salt_level || '—'} color={saltOk ? TOKENS_LIGHT.colors.success : TOKENS_LIGHT.colors.warning} />
          <TankMini label="ÚLT. EXTR." value={t.last_extraction_time ? formatOdooTime(t.last_extraction_time) : '—'} color={TOKENS_LIGHT.colors.blue2} />
        </div>
        {hasAlerts && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
            <span style={{ fontSize: 12 }}>&#x26A0;</span>
            <span style={{ ...typo.caption, color: TOKENS_LIGHT.colors.error, fontWeight: 600 }}>
              {[
...
function TankMini({ label, value, color }) {
  return (
    <div style={{ textAlign: 'center' }}>
      <p style={{ fontSize: 14, fontWeight: 800, color: color || TOKENS_LIGHT.colors.textMuted, margin: 0 }}>{value}</p>
      <p style={{ fontSize: 9, fontWeight: 600, color: TOKENS_LIGHT.colors.textMuted, margin: 0, marginTop: 2 }}>{label}</p>
    </div>
  )
}

function getReadingStatusColor(kind) {
  if (kind === 'ok') return TOKENS_LIGHT.colors.success
  if (kind === 'low') return TOKENS_LIGHT.colors.error
  if (kind === 'stale') return TOKENS_LIGHT.colors.warning
  return TOKENS_LIGHT.colors.textMuted
}

function formatReadingTimestamp(value) {
  if (!value) return '—'
  return formatOdooDateTime(value) || String(value).slice(0, 16)
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {items.map(a => (
        <button key={a.id} onClick={() => onGo(a.route)} style={{
          ...
          <div style={{ flex: 1 }}>
            <p style={{ ...typo.title, color: TOKENS_LIGHT.colors.text, margin: 0 }}>{a.label}</p>
            <p style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, margin: '2px 0 0' }}>{a.desc}</p>
          </div>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={TOKENS_LIGHT.colors.textMuted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6"/></svg>
        </button>
      ))}
    </div>
  )

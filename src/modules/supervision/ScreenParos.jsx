    ...
    <div style={{
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 20, paddingBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <span style={{ ...typo.title, color: TOKENS_LIGHT.colors.textSoft }}>Paros de Linea</span>
          </div>
          <span style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted }}>{openDt.length} activos</span>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ marginTop: 40, padding: 24, borderRadius: TOKENS.radius.xl, background: TOKENS_LIGHT.glass.panel, border: `1px solid ${TOKENS_LIGHT.colors.border}`, textAlign: 'center' }}>
            ...
            <p style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, marginTop: 6 }}>Abre un turno para poder registrar paros.</p>
          ...
          <>
            ...
            {!showForm ? (
              ...
              <form onSubmit={handleCreate} style={{
                ...
                <p style={{ ...typo.title, color: TOKENS_LIGHT.colors.text, margin: '0 0 12px' }}>Nuevo Paro</p>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  {voiceNote && (
                    <div style={{
                      ...
                      background: TOKENS_LIGHT.colors.warningSoft, border: '1px solid rgba(245,158,11,0.25)',
                    }}>
                      <p style={{ ...typo.caption, color: TOKENS_LIGHT.colors.warning, margin: 0 }}>{voiceNote}</p>
                    </div>
                  )}
                </div>

                <label style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, display: 'block', marginBottom: 4 }}>Categoria</label>
                <select value={formData.category_id} onChange={e => setFormData(p => ({ ...p, category_id: e.target.value }))}
                  ...
                  <option value="" style={{ color: '#111827', background: '#ffffff' }}>Seleccionar...</option>
                  {categories.map(c => <option key={c.id} value={c.id} style={{ color: '#111827', background: '#ffffff' }}>{c.name}</option>)}
                </select>
                ...
                <select value={formData.line_id} onChange={e => setFormData(p => ({ ...p, line_id: e.target.value }))}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: TOKENS.radius.sm, background: TOKENS_LIGHT.colors.surfaceSoft, border: `1px solid ${TOKENS_LIGHT.colors.border}`, color: TOKENS_LIGHT.colors.text, fontSize: 13, fontFamily: 'inherit', marginBottom: 10 }}>
                  <option value="" style={{ color: '#111827', background: '#ffffff' }}>Seleccionar...</option>
                  {lines.map(l => <option key={l.id} value={l.id} style={{ color: '#111827', background: '#ffffff' }}>{l.name}</option>)}
                </select>

                <label style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, display: 'block', marginBottom: 4 }}>Maquina (opcional)</label>
                <select value={formData.machine_id} onChange={e => setFormData(p => ({ ...p, machine_id: e.target.value }))}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: TOKENS.radius.sm, background: TOKENS_LIGHT.colors.surfaceSoft, border: `1px solid ${TOKENS_LIGHT.colors.border}`, color: TOKENS_LIGHT.colors.text, fontSize: 13, fontFamily: 'inherit', marginBottom: 10 }}>
                  <option value="" style={{ color: '#111827', background: '#ffffff' }}>Sin especificar</option>
                  {machines.map(m => <option key={m.id} value={m.id} style={{ color: '#111827', background: '#ffffff' }}>{m.name}</option>)}
                </select>

                <label style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, display: 'block', marginBottom: 4 }}>Responsable</label>
                <input type="text" value={formData.responsible}
                  onChange={e => setFormData(p => ({ ...p, responsible: e.target.value }))}
                  placeholder="Quien reporta"
                  style={{ width: '100%', padding: '10px 12px', borderRadius: TOKENS.radius.sm, background: TOKENS_LIGHT.colors.surfaceSoft, border: `1px solid ${TOKENS_LIGHT.colors.border}`, color: TOKENS_LIGHT.colors.text, fontSize: 13, fontFamily: 'inherit', marginBottom: 10 }} />

                <label style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, display: 'block', marginBottom: 4 }}>
                  Motivo <span style={{ color: TOKENS_LIGHT.colors.error }}>*</span>
                </label>
                ...
                <label style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, display: 'block', marginBottom: 4 }}>Comentario adicional (opcional)</label>
            ...
            {openDt.length > 0 && (
              <>
                <div style={{
                  ...
                  <p style={{ ...typo.overline, color: TOKENS_LIGHT.colors.error, margin: 0, fontWeight: 800 }}>
                    PAROS ACTIVOS
                  </p>
                  <span style={{
                    fontSize: 11, fontWeight: 700, color: TOKENS_LIGHT.colors.error,
                  ...
                  {openDt.map(d => {
                    ...
                    <div key={d.id} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                        <div style={{ flex: 1 }}>
                          <p style={{ ...typo.title, color: TOKENS_LIGHT.colors.text, margin: 0 }}>{d.category || d.name || 'Paro'}</p>
                        ...
                        <div style={{ padding: '4px 10px', borderRadius: TOKENS.radius.pill, background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.4)' }}>
                          <span style={{ fontSize: 11, fontWeight: 700, color: TOKENS_LIGHT.colors.error }}>ACTIVO</span>
                        </div>
                      </div>
                      {d.reason && <p style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, margin: '0 0 10px' }}>{d.reason}</p>}
                      <button onClick={() => handleClose(d.id)} disabled={processing === d.id}
                        style={{
                          ...
                          color: TOKENS_LIGHT.colors.error, fontSize: 14, fontWeight: 700,
            ...
            {closedDt.length > 0 && (
              <>
                <p style={{ ...typo.overline, color: TOKENS_LIGHT.colors.textLow, marginBottom: 8 }}>PAROS CERRADOS</p>
                    ...
                    <div key={d.id} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        <div style={{ padding: '4px 10px', borderRadius: TOKENS.radius.pill, background: 'rgba(34,197,94,0.12)', border: '1px solid rgba(34,197,94,0.25)' }}>
                          <span style={{ fontSize: 11, fontWeight: 700, color: TOKENS_LIGHT.colors.success }}>CERRADO</span>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            {downtimes.length === 0 && (
              <div style={{ marginTop: 20, padding: 24, borderRadius: TOKENS.radius.xl, background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)', textAlign: 'center' }}>
                ...
                <p style={{ ...typo.title, color: TOKENS_LIGHT.colors.success }}>Sin paros</p>
                <p style={{ ...typo.caption, color: TOKENS_LIGHT.colors.textMuted, marginTop: 6 }}>No hay paros registrados en este turno.</p>
              </div>
            )}
          </>
        )}

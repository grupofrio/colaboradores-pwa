// ─── KOLD OS · M2 — Cliente autenticado de la API (mecanismo canónico) ───────
// Consume EXCLUSIVAMENTE los endpoints GET autenticados de gf_kold_os_m2 vía
// api() (X-GF-Employee-Token, /odoo-api, ApiError status/code, sin retries,
// PROHIBIDO fallback n8n — ver directKoldOsM2 en src/lib/api.js).
//
...
import { KOLD_OS_M2_LATEST_PATH, KOLD_OS_M2_FINDINGS_PATH, KOLD_OS_M2_FINDINGS_PARAMS } from '../../../lib/koldOsM2Route.js'
import { validateM2Latest, validateM2Findings } from './contract.js'
...
export const M2_TIMEOUT_MS = 30000
...
export const M2_MAX_PAYLOAD_CHARS = 2_000_000

export function withTimeout(promise, ms = M2_TIMEOUT_MS) {
  let timer
...
export function classifyM2Error(err) {
...
function guardSize(payload) {
  if (JSON.stringify(payload).length > M2_MAX_PAYLOAD_CHARS) {
...
/**
 * GET /pwa-kold-os/m2/latest → { state: 'ok', payload } | { state, errors? }.
 * Estados: ok · disabled · session_expired · forbidden · unavailable ·
 * schema_mismatch · invalid · error.
 */
export async function fetchM2Latest({ timeoutMs = M2_TIMEOUT_MS, apiImpl = defaultApi } = {}) {
  let payload
  try {
    payload = guardSize(await withTimeout(apiImpl('GET', KOLD_OS_M2_LATEST_PATH), timeoutMs))
  } catch (err) {
    return { ...classifyM2Error(err), errors: [String(err?.code || err?.message || 'error')] }
  }
  const { ok, errors, schema } = validateM2Latest(payload)
  if (!ok) {
...
/** GET /pwa-kold-os/m2/findings con filtros del contrato (paginado server-side). */
export async function fetchM2Findings(params = {}, { timeoutMs = M2_TIMEOUT_MS, apiImpl = defaultApi } = {}) {
  ...
  let payload
  try {
    payload = guardSize(await withTimeout(apiImpl('GET', `${KOLD_OS_M2_FINDINGS_PATH}${suffix}`), timeoutMs))
  } catch (err) {
    return { ...classifyM2Error(err), errors: [String(err?.code || err?.message || 'error')] }
  }
  const { ok, errors, schema } = validateM2Findings(payload)
  if (!ok) {
    return schema === 'unsupported'
      ? { state: 'schema_mismatch', errors }
      : { state: 'invalid', errors }

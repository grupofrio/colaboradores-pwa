...
export function createMultiCompanyExpense(scope, data = {}) {
  return { ...requireScope(scope), ...businessFields(data) }
}

export function submitMultiCompanyExpense(scope, data = {}) {
  return api('POST', '/pwa-admin/expense-create', createMultiCompanyExpense(scope, data))
}

export function getMultiCompanyExpenses(scope) {
  const selected = requireScope(scope)
  return api('GET', `/pwa-admin/today-expenses?operating_company_id=${encodeURIComponent(selected.operating_company_id)}&operating_plaza_id=${encodeURIComponent(selected.operating_plaza_id)}`)
}

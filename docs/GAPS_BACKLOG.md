...
- **Proxy SalesOps en Vercel**: `GF_SALESOPS_TOKEN` se configura sólo del lado servidor; si falta o no coincide, los endpoints `/gf/salesops/*` responden `UNAUTHORIZED`.
...
- **G002** — Título original "JWT local unsigned (alg:'none')" era incorrecto. **El sistema NO usa JWT.** Las credenciales por empleado son opacas y el secreto de SalesOps es estático en `ir.config_parameter`. El vector real era privilege escalation en `gf_saleops/services/guard.py:52` que derivaba el rol del `employee_id` enviado en body, combinado con el secreto global de SalesOps. Renombrado y cerrado tras fix de Sebastián entre 2026-04-29 y 2026-05-05.
